...
TCP/IP 意味着 TCP 和 IP 在一起[协同工作](https://www.runoob.com/tcpip/tcpip-intro.html)。
...
![tpc-IP-data-struct](images/tpc-IP-data-struct.png)
...
12 个阿拉伯数字很难记忆。使用一个名称更容易。即域名的来历。
...
UDP报文没有可靠性保证、顺序保证和流量控制字段等，可靠性较差。但是正因为UDP协议的控制选项较少，在数据传输过程中延迟小、[数据传输效率](https://baike.baidu.com/item/数据传输效率/1749729)高，适合对可靠性要求不高的应用程序，或者可以保障可靠性的应用程序，如DNS、TFTP、SNMP等。
