...
- 事务隔离级别
  ...
  - 可重复读（`repeatable read`）：MySQL默认隔离级别，解决了脏读问题。该级别保证了在同一个事务中，多次读取同样记录的结果是一致的！但理论上无法解决幻读（`phantom read`)。幻读指的是当某个事务在读取某个范围内的记录时，另外一个事务又在该范围内插入了新的记录，当之前的事务再次读取该范围的记录时，会产生幻行（`phantom row`）。`InnoDB`存储引擎通过多版本并发控制（`MVCC，Multiversion Concurrency Control`）解决了幻读的问题。
  - 可串行化（`serialization`）：最高的隔离级别。通过强制事务串行执行，避免了幻读。`serialization`会在读取的每一行数据上都加锁（所以可能导致大量的超时和锁争用问题）