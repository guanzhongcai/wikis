...
2. **算法Algorithms**，如sort、find、copy、for_each。从实现的角度来看，STL算法是一种 function template
...
如果重载的 operator() 形参是两个参数，叫二元仿函数（binary functor）；
...
- OOP是把data和method结合在一起

- GP是把data和method分开，例如vector/list和sort/min/max/find/..
...
![](houjie/stl-struct-classify.png)



- heap并不归属于STL容器组件，它是个幕后英雄，扮演priority queue的助手。顾名思义，priority queue允许用户以任何次序将任何元素推入容器内，但取出时一定是从优先权最高（也就是数值最高）的元素开始取。binary max heap 正是具有这样的特性，适合作为priority queue 的底层机制。（[STL——heap结构及算法](https://www.cnblogs.com/yyxt/p/4979681.html)）
