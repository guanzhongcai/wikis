...
- 需要新增两个本地的环境变量存到bat中，表示增量和全量是否已经开启，默认不开启
- 启动时自动读取bat中环境变量，来自动创建cron任务
- 配置更新后，触发启动时的自动创建cron任务
- 根据时间段，换算成fixing的周期任务，判断当前时间是否在此时间范围内，yes就执行
- 全量是每天的某个固定时间执行
- 增量是从开始时间开始，在一天内持续执行的，可以每分支执行for循环执行

参考资料

- [Java的任务调度，使用Cron表达式实现](https://www.cnblogs.com/qiutianyou/p/5513167.html)
- 
